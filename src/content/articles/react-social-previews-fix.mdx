...
Instead of the beautiful preview card you imagined—your carefully chosen cover image, your compelling title—you get... your site's generic logo. Or worse, nothing at all. A blank card with just a URL staring back at you.
...
> **🤔 Why Edge Functions Beat Pre-rendering Services**
>
...
// ✅ RIGHT - filename is the single source of truth
const slug = file.replace('.mdx', '');
```
...
**The fix:** In Cloudflare dashboard → Scrape Shield → Hotlink Protection, either:
...
- [opengraph.xyz](https://opengraph.xyz) — Clean interface, shows previews for multiple platforms
- [metatags.io](https://metatags.io) — Similar, with a handy code generator
...
-  **Image URL is absolute** — `https://yoursite.com/image.png`, not `/image.png`
-  **Image is publicly reachable** — Open the URL in incognito. Can you see it without logging in?
-  **Image dimensions are 1200x630** — The recommended OG image size
-  **Image is under 5MB** — Some platforms reject larger files
-  **Title is under 60 characters** — Gets truncated otherwise
-  **Description is under 160 characters** — Same reason
-  **No trailing slash mismatches** — `/blog/post` and `/blog/post/` might be cached separately
-  **HTTPS everywhere** — Some platforms reject HTTP images
-  **No hotlink protection blocking bots** — Check Cloudflare/CDN settings if images aren't loading
...
- **Dynamic OG image generation** — Tools like [Vercel OG](https://vercel.com/docs/functions/og-image-generation) can generate images on-the-fly. That's a separate (and worthwhile) topic.
- **Next.js/Remix/Astro solutions** — If you're using a framework with built-in SSR, you don't need this edge function approach. Your meta tags are already in the initial HTML.
- **Non-Vercel deployments** — The edge function pattern works elsewhere (Cloudflare Workers, Netlify Edge Functions), but the configuration differs.
